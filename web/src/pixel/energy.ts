...
export function gradEnergy(gray01: Float32Array, width: number, height: number, sigma: number): Float32Array {
  ...
  for (let i = 0; i < e.length; i++) e[i] = Math.abs(gx[i]) + Math.abs(gy[i]);
  return e;
}
...
export function enhanceEnergyDirectional(
  ...
  for (let i = 0; i < out.length; i++) {
    let v = energy[i];
    if (horizontalFactor > 1) v += Math.abs(gy[i]) * (horizontalFactor - 1);
    if (verticalFactor > 1) v += Math.abs(gx[i]) * (verticalFactor - 1);
    out[i] = v;
  }

  // clip 到 p99.9（这里同样用近似 quantile）
  const clipV = quantileApprox(out, 0.999);
  for (let i = 0; i < out.length; i++) out[i] = Math.min(out[i], clipV);
